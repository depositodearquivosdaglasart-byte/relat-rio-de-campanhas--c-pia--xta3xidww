...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    return new Response(JSON.stringify({ error: 'Configuração de ambiente ausente' }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
  ...
  try {
    ...
    if (!diarios || diarios.length === 0) {
      return new Response(JSON.stringify({ success: true, message: 'Nenhum dado diário encontrado para agregar' }), {
    ...
    return new Response(JSON.stringify({ success: true, message: `Foram gerados ${count} registros consolidados.` }), {
      headers: { 'Content-Type': 'application/json' },
    })
  } catch (error: any) {
